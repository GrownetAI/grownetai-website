...
/* Bound the map: drop IPs whose newest hit fell out of the window, then
   evict oldest-inserted entries past the hard cap so spoofed
   X-Forwarded-For values can't grow memory without bound. */
function sweepLog(now: number) {
  for (const [key, times] of submissionLog) {
    if (times.length === 0 || now - times[times.length - 1] >= WINDOW_MS) {
      submissionLog.delete(key);
    }
  }
  while (submissionLog.size > MAX_TRACKED_IPS) {
...
function recordSubmission(ip: string) {
  ...
  // Delete-then-set keeps Map insertion order tracking recency for eviction
...
async function sendEmail(data: z.infer<typeof schema>) {
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <div style="background: #008080; padding: 30px; border-radius: 12px 12px 0 0;">
        <h1 style="color: white; margin: 0; font-size: 24px;">New Project Brief</h1>
        <p style="color: rgba(255,255,255,0.8); margin: 8px 0 0;">GrownetAI Website — What we do</p>
      </div>
      <div style="background: #f9fafb; padding: 30px; border-radius: 0 0 12px 12px; border: 1px solid #e5e7eb;">
        <table style="width: 100%; border-collapse: collapse;">
          ...
          <tr><td style="padding: 10px 0; border-bottom: 1px solid #e5e7eb; color: #6b7280; font-weight: 600;">Business type</td><td style="padding: 10px 0; border-bottom: 1px solid #e5e7eb; color: #111827;">${data.businessType ? sanitize(data.businessType) : "—"}</td></tr>
          <tr><td style="padding: 10px 0; color: #6b7280; font-weight: 600; vertical-align: top;">Project details</td><td style="padding: 10px 0; color: #111827; white-space: pre-wrap;">${sanitize(data.message)}</td></tr>
        </table>
  ...
  await transporter.sendMail({
    ...
    to: process.env.CONTACT_RECIPIENT_EMAIL || "contact@grownetai.com",
    replyTo: data.email,
    subject: `New project brief from ${data.name} — GrownetAI`,
    html,
  });
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Send email; only a successful send consumes a rate-limit slot
    await sendEmail(parsed.data);
    recordSubmission(ip);

    console.log("[Brief] Email sent from:", parsed.data.email, "| IP:", ip);

    return NextResponse.json(
      { success: true, message: "Brief received! We'll reply within 24 hours." },
      { status: 200 },
    );
  } catch (err) {
