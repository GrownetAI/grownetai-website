    ...
    <main className="pt-[var(--navbar-height)]">
      ...
      <section className="relative bg-gradient-hero py-20 overflow-hidden">
        ...
        <div className="container-site relative z-10 text-center">
          <span className="section-label bg-white/20 text-white border-white/30 mb-4">Blog & Insights</span>
          ...
          <p className="text-white/80 text-xl max-w-xl mx-auto">
            AI-powered strategies, industry trends, and actionable growth tactics for your business.
          </p>
        </div>
      </section>
              ...
              <Link href={`/blog/${featuredPost.slug}`} className="group block">
                <div className="card card-hover overflow-hidden">
                  <div className="grid grid-cols-1 lg:grid-cols-2">
                    <div className="bg-gradient-brand h-64 lg:h-auto flex items-center justify-center min-h-[260px]">
                      <BookOpen className="w-16 h-16 text-white/40" />
                    </div>
                    <div className="p-8 lg:p-10 flex flex-col justify-center">
                      <div className="flex items-center gap-3 mb-4">
                        <span className="badge-teal">{featuredPost.category}</span>
                      ...
                      <p className="text-brand-slate-gray leading-relaxed mb-6">{featuredPost.excerpt}</p>
      ...
      <section className="section-padding bg-brand-cloud-white">
        <div className="container-site">
              ...
              <FadeIn key={post.id} delay={i * 0.06}>
                <Link href={`/blog/${post.slug}`} className="group block h-full">
                  <div className="card card-hover h-full flex flex-col overflow-hidden">
                    <div className="bg-gradient-to-br from-brand-teal-mist to-brand-mint-cream h-44 flex items-center justify-center">
                      <BookOpen className="w-10 h-10 text-brand-teal/30" />
                    </div>
                    <div className="p-6 flex flex-col flex-1">
                      <span className="badge-teal self-start mb-3">{post.category}</span>
                      ...
                      <p className="text-brand-slate-gray text-sm leading-relaxed mb-4">{post.excerpt}</p>
                      <div className="flex items-center gap-3 text-xs text-brand-slate-gray border-t border-gray-100 pt-4">
                        <span className="flex items-center gap-1"><User className="w-3.5 h-3.5" /> {post.author}</span>
                        <span className="flex items-center gap-1 ml-auto"><Clock className="w-3.5 h-3.5" /> {post.readTime}</span>
                      </div>
                    </div>
                  </div>
                </Link>
          ...
          {filtered.length === 0 && (
            <p className="text-center text-brand-slate-gray mt-10">No posts in this category yet. Check back soon!</p>
          )}
        </div>
      </section>
      ...
      <section className="section-padding bg-brand-teal text-white text-center">
        <div className="container-site max-w-xl mx-auto">
          <h2 className="font-heading font-bold text-3xl md:text-4xl mb-4">Want us to grow your business too?</h2>
          <p className="text-white/80 mb-8">Join 150+ businesses already growing with GrownetAI.</p>
          <Link href="/contact" className="btn btn-lg bg-white text-brand-teal font-bold hover:bg-white/90">Get Free Consultation</Link>
        </div>
      </section>
    </main>
  );
